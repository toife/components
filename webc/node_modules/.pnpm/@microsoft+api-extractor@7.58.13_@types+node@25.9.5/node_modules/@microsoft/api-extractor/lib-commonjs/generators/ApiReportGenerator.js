...
const SyntaxHelpers_1 = require("../analyzer/SyntaxHelpers");
const ExtractorMessageId_1 = require("../api/ExtractorMessageId");
const _trimSpacesRegExp = / +$/gm;
class ApiReportGenerator {
    ...
    static generateReviewFileContent(collector, reportVariant) {
        ...
        for (const entity of collector.entities) {
            ...
            const maxEffectiveReleaseTag = symbolMetadata?.maxEffectiveReleaseTag ?? api_extractor_model_1.ReleaseTag.None;
                ...
                if (astEntity instanceof AstNamespaceImport_1.AstNamespaceImport) {
                    ...
                    for (const [exportedName, exportedEntity] of astModuleExportInfo.exportedLocalEntities) {
                        ...
                        if (collectorEntity.nameForEmit === exportedName) {
                            ...
                            const safeExportedName = SyntaxHelpers_1.SyntaxHelpers.isSafeUnquotedMemberIdentifier(exportedName)
                                ? exportedName
        ...
        if (unassociatedMessages.length > 0) {
            ...
            for (const unassociatedMessage of unassociatedMessages) {
                _writeLineAsComments(writer, unassociatedMessage.formatMessageWithLocation(collector.workingPackage.packageFolder));
            }
        }
 ...
 */
function _modifySpan(span, entity, astDeclaration, insideTypeLiteral, context) {
...
function _shouldIncludeDeclaration(collector, astDeclaration, reportVariant) {
...
function _shouldIncludeReleaseTag(releaseTag, reportVariant) {
 ...
 */
function _modifySpanForPreapproved(span) {
 ...
 * by the analysis.
 */
function _getAedocSynopsis(collector, astDeclaration, messagesToReport) {
    const writer = new IndentedWriter_1.IndentedWriter();
...
function _writeLineAsComments(writer, line) {
