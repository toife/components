...
exports.condenseTokens = condenseTokens;
 ...
 */
function condenseTokens(excerptTokens, tokenRanges) {
    const originalTokenCount = excerptTokens.length;
    ...
    const newIndexByOriginalIndex = new Int32Array(originalTokenCount + 1);
    ...
    let prevTokenIsPreserved = false;
    for (let currentIndex = 0; currentIndex < originalTokenCount; ++currentIndex) {
        const currentToken = excerptTokens[currentIndex];
        const currentIsPreserved = preservedIndices.has(currentIndex);
        const condensedCount = condensedTokens.length;
        ...
        if (!currentIsPreserved && condensedCount >= 1) {
            const prevToken = condensedTokens[condensedCount - 1];
            if (condensedCount >= 2 &&
                ...
                condensedTokens[condensedCount - 2].kind === api_extractor_model_1.ExcerptTokenKind.Reference) {
                ...
                const prevPrevToken = condensedTokens[condensedCount - 2];
            ...
            prevToken.kind === api_extractor_model_1.ExcerptTokenKind.Content &&
                currentToken.kind === api_extractor_model_1.ExcerptTokenKind.Content) {
...
//# sourceMappingURL=condenseTokens.js.map