...
const _csiRegExp = /\x1b\[([\x30-\x3f]*[\x20-\x2f]*[\x40-\x7e])/gu;
...
const _sgrRegExp = /([0-9]+)m/u;
const _backslashNRegExp = /\n/g;
const _backslashRRegExp = /\r/g;
/**
...
export class AnsiEscape {
    ...
    static formatForTests(text, options) {
        ...
        let result = text.replace(_csiRegExp, (capture, csiCode) => {
            // If it is an SGR code, then try to show a friendly token
            const match = csiCode.match(_sgrRegExp);
            if (match) {
                const sgrParameter = parseInt(match[1], 10);
                const sgrParameterName = _tryGetSgrFriendlyName(sgrParameter);
                if (sgrParameterName) {
                    // Example: "[black-bg]"
...
function _tryGetSgrFriendlyName(sgiParameter) {
    switch (sgiParameter) {
