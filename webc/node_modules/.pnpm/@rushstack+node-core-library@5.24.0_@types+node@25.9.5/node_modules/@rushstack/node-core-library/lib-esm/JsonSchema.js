...
export class JsonSchema {
    ...
    _schemaObject = undefined;
    _schemaVersion = undefined;
    _customFormats = undefined;
    _rejectVendorExtensionKeywords = false;
    constructor() { }
    /**
    ...
    ensureCompiled() {
        this._ensureLoaded();
        if (!this._validator) {
            const targetSchemaVersion = this._schemaVersion ?? _inferJsonSchemaVersion(this._schemaObject);
    ...
    validateObject(jsonObject, filenameForErrors, options) {
        this.validateObjectWithCallback(jsonObject, (errorInfo) => {
            const prefix = options?.customErrorHeader ?? 'JSON validation failed:';
    ...
    validateObjectWithCallback(jsonObject, errorCallback, options) {
        ...
        if (this._validator && !this._validator(jsonObject)) {
            const errorDetails = _formatErrorDetails(this._validator.errors);
            const args = {
    ...
    _collectDependentSchemas(collectedSchemas, dependentSchemas, seenObjects, seenIds) {
        for (const dependentSchema of dependentSchemas) {
            ...
            this._collectDependentSchemas(collectedSchemas, dependentSchema._dependentSchemas, seenObjects, seenIds);
        }
    }
}
/**
 ...
 */
function _formatErrorDetailsHelper(errorDetails, indent, buffer) {
