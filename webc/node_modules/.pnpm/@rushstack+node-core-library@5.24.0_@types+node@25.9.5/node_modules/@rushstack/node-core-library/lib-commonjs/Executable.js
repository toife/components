...
class Executable {
    ...
    static spawnSync(filename, args, options) {
        ...
        const context = _getExecutableContext(options);
        const resolvedPath = _tryResolve(filename, options, context);
        if (!resolvedPath) {
        ...
        const normalizedCommandLine = _buildCommandLineFixup(resolvedPath, args, context);
...
function _tryResolve(filename, options, context) {
    ...
    if (hasPathSeparators) {
        ...
        const pathsToSearch = _getSearchFolders(context);
        for (const pathToSearch of pathsToSearch) {
            const resolvedPath = path.join(pathToSearch, filename);
            const result = _tryResolveFileExtension(resolvedPath, context);
            if (result) {
                return result;
            }
        }
...
function _tryResolveFileExtension(resolvedPath, context) {
...
function _buildEnvironmentMap(options) {
 ...
 */
function _canExecute(filePath, context) {
 ...
 */
function _getSearchFolders(context) {
...
function _getExecutableContext(options) {
    if (!options) {
        options = {};
    }
    const environment = _buildEnvironmentMap(options);
 ...
 */
function _getEscapedForWindowsShell(text) {
 ...
 */
function _validateArgsForWindowsShell(args) {
...
function _buildCommandLineFixup(resolvedPath, args, context) {
