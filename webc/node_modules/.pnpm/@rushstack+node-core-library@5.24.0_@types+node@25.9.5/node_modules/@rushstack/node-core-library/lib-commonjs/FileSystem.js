...
function _handleLinkExistError(linkFn, options, error) {
...
function _handleLink(linkFn, options) {
    try {
        linkFn();
    }
    catch (error) {
        if (FileSystem.isExistError(error)) {
            ...
            _handleLinkExistError(linkFn, options, error);
        }
        else {
            ...
            if (FileSystem.isNotExistError(error) &&
                ...
                try {
                    ...
                    if (FileSystem.isExistError(retryError)) {
                        ...
                        _handleLinkExistError(linkFn, options, retryError);
                    }
                    else {
                        throw retryError;
                    }
...
async function _handleLinkExistErrorAsync(linkFn, options, error) {
    switch (options.alreadyExistsBehavior) {
        case AlreadyExistsBehavior.Ignore:
            break;
        case AlreadyExistsBehavior.Overwrite:
...
async function _handleLinkAsync(linkFn, options) {
    try {
        await linkFn();
    }
    catch (error) {
        if (FileSystem.isExistError(error)) {
            // Link exists, handle it
            await _handleLinkExistErrorAsync(linkFn, options, error);
        }
        else {
            // When attempting to create a link in a directory that does not exist, an ENOENT
            // or ENOTDIR error is thrown, so we should ensure the directory exists before
            ...
            if (FileSystem.isNotExistError(error) &&
                ...
                try {
                    ...
                    if (FileSystem.isExistError(retryError)) {
                        ...
                        // call and the retry; handle it the same way as the initial exist error.
                        await _handleLinkExistErrorAsync(linkFn, options, retryError);
                    }
                    else {
                        throw retryError;
                    }
...
async function _wrapExceptionAsync(fn) {
    try {
        return await fn();
    }
    catch (error) {
