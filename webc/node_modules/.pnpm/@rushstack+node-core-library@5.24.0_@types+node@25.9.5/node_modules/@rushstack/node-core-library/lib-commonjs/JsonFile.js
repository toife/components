...
class JsonFile {
     ...
     * @internal
     */
    static _formatPathForError = (path) => path;
    /**
     * Loads a JSON file.
     */
    static load(jsonFilename, options) {
        try {
            ...
            const parseOptions = _buildJjuParseOptions(options);
...
function _validateNoUndefinedMembers(jsonObject, keyPath) {
    if (!jsonObject) {
        return;
    }
    if (typeof jsonObject === 'object') {
        for (const key of Object.keys(jsonObject)) {
            ...
            if (value === undefined) {
                const fullPath = _formatKeyPath(keyPath);
...
function _formatKeyPath(keyPath) {
    let result = '';
...
function _formatJsonHeaderComment(headerComment) {
    if (headerComment === '') {
        return '';
    }
    const lines = Text_1.Text.convertToLf(headerComment).split('\n');
...
function _buildJjuParseOptions(options = {}) {
    const parseOptions = {
