...
exports._setLockFileGetProcessStartTime = _setLockFileGetProcessStartTime;
...
class LockFile {
    ...
    static tryAcquire(resourceFolder, resourceName) {
        ...
        const result = _tryAcquireInner(resourceFolder, resourceName, lockFilePath);
...
function _tryAcquireInner(resourceFolder, resourceName, lockFilePath) {
 ...
 */
function _tryAcquireMacOrLinux(resourceFolder, resourceName, pidLockFilePath) {
    ...
    const startTime = _getStartTime(pid);
    if (!startTime) {
    ...
    try {
        ...
        for (const fileInFolder of files) {
            if ((match = fileInFolder.match(lockFileRegExp)) &&
                ...
                const otherPidCurrentStartTime = _getStartTime(parseInt(otherPid, 10));
 ...
 */
function _tryAcquireWindows(lockFilePath) {
    let dirtyWhenAcquired = false;
    let fileHandle;
    let result;
    try {
