 ...
 */
const _subprocessesByPid = new Map();
const _isWindows = node_process_1.default.platform === 'win32';
/**
...
function _ensureInitialized() {
    if (!_initialized) {
        ...
        node_process_1.default.prependListener('SIGTERM', _onTerminateSignal);
        node_process_1.default.prependListener('SIGINT', _onTerminateSignal);
        node_process_1.default.prependListener('exit', _onExit);
    }
}
// Uninstall the hooks and perform cleanup
function _cleanupChildProcesses() {
    if (_initialized) {
        _initialized = false;
        node_process_1.default.removeListener('SIGTERM', _onTerminateSignal);
        node_process_1.default.removeListener('SIGINT', _onTerminateSignal);
        const trackedSubprocesses = Array.from(_subprocessesByPid.values());
...
function _validateSubprocessOptions(subprocessOptions) {
    if (!_isWindows) {
...
function _onTerminateSignal(signal) {
