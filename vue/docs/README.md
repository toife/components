# Tài liệu @toife/vue

- [common.md](./common.md) — Cài đặt, peer dependencies, đăng ký component, Sass
- [types.md](./types.md) — Chi tiết object/mảng trong props, emit, provide (GridOption, ActionButton, …)

## Components

| File | Component | Tag (ví dụ `t-`) |
|------|-----------|------------------|
| [app.md](./app.md) | App | `t-app` |
| [action.md](./action.md) | Action | `t-action` |
| [avatar.md](./avatar.md) | Avatar | `t-avatar` |
| [button.md](./button.md) | Button | `t-button` |
| [cable.md](./cable.md) | Cable | `t-cable` |
| [card.md](./card.md) | Card | `t-card` |
| [card-header.md](./card-header.md) | CardHeader | `t-card-header` |
| [card-body.md](./card-body.md) | CardBody | `t-card-body` |
| [card-footer.md](./card-footer.md) | CardFooter | `t-card-footer` |
| [checkbox.md](./checkbox.md) | Checkbox | `t-checkbox` |
| [collapse.md](./collapse.md) | Collapse | `t-collapse` |
| [container.md](./container.md) | Container | `t-container` |
| [decision-modal.md](./decision-modal.md) | DecisionModal | `t-decision-modal` |
| [divider.md](./divider.md) | Divider | `t-divider` |
| [dropdown.md](./dropdown.md) | Dropdown | `t-dropdown` |
| [field.md](./field.md) | Field | `t-field` |
| [flex.md](./flex.md) | Flex | `t-flex` |
| [flex-item.md](./flex-item.md) | FlexItem | `t-flex-item` |
| [form-group.md](./form-group.md) | FormGroup | `t-form-group` |
| [gesture-indicator.md](./gesture-indicator.md) | GestureIndicator | `t-gesture-indicator` |
| [grid.md](./grid.md) | Grid | `t-grid` |
| [grid-item.md](./grid-item.md) | GridItem | `t-grid-item` |
| [image.md](./image.md) | Image | `t-image` |
| [modal.md](./modal.md) | Modal | `t-modal` |
| [page.md](./page.md) | Page | `t-page` |
| [present.md](./present.md) | Present | `t-present` |
| [progress.md](./progress.md) | Progress | `t-progress` |
| [radio.md](./radio.md) | Radio | `t-radio` |
| [radio-group.md](./radio-group.md) | RadioGroup | `t-radio-group` |
| [refresher.md](./refresher.md) | Refresher | `t-refresher` |
| [route-wrapper.md](./route-wrapper.md) | RouteWrapper | `t-route-wrapper` |
| [route-provider.md](./route-provider.md) | RouteProvider | `t-route-provider` |
| [route-navigator.md](./route-navigator.md) | RouteNavigator | `t-route-navigator` |
| [route-outlet.md](./route-outlet.md) | RouteOutlet | `t-route-outlet` |
| [scrollbar.md](./scrollbar.md) | Scrollbar | `t-scrollbar` |
| [segmented-field.md](./segmented-field.md) | SegmentedField | `t-segmented-field` |
| [select.md](./select.md) | Select | `t-select` |
| [slide-range.md](./slide-range.md) | SlideRange | `t-slide-range` |
| [skeleton.md](./skeleton.md) | Skeleton | `t-skeleton` |
| [switch.md](./switch.md) | Switch | `t-switch` |
| [tab.md](./tab.md) | Tab | `t-tab` |
| [tabs.md](./tabs.md) | Tabs | `t-tabs` |
| [toast.md](./toast.md) | Toast | `t-toast` |
| [toast-content.md](./toast-content.md) | ToastContent | `t-toast-content` |
| [toolbar.md](./toolbar.md) | Toolbar | `t-toolbar` |
| [virtual-list.md](./virtual-list.md) | VirtualList | `t-virtual-list` |
